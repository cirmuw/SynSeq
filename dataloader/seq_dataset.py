...
class SyntaxDataset(Dataset):
    def __init__(
        ...
        ## Apply inverse frequency weighing function
        labels = torch.tensor([int(rec[self.label]) for rec in dataset])  # values in [0, 70]

        counts = torch.bincount(labels, minlength=71)

        # inverse frequency
        weights = 1.0 / (counts.float() + 1e-6)

        for rec in dataset:
            rec["weight"] = weights[int(rec[self.label])]
        if self.train:
            ...
            if isinstance(healthy_weight, (int, float)):
                for rec in self.dataset:
                    rec["weight"] = 1.0
                for rec in self.negative_dataset:
                    rec["weight"] = healthy_weight
    ...
    def __getitem__(self, idx: int) -> Tuple[Tensor, int]:
        ...
        if self.use_log_space:
            ...
            target = torch.tensor([rec[self.label]/70], dtype=torch.float32)
        ...
        for vi in seq:
            ...
            video = torch.tensor(video).unsqueeze(-1)  # for complying with ToTensorVideo unsqueeze -1
